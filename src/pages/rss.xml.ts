...
export async function GET({ site }: { site: URL }) {
  ...
  const items = articles.map((article) => `<item><title>${escape(article.data.title)}</title><link>${new URL(articlePath(article), site)}</link><guid>${new URL(articlePath(article), site)}</guid><description>${escape(article.data.description)}</description><pubDate>${article.data.publishedAt.toUTCString()}</pubDate></item>`).join('');
